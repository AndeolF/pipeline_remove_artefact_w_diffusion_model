...
# class FeaturesCondition(nn.Module):
...
class Transformer(nn.Module):
    def __init__(self):
        ...
        self.linear_emb_to_patch = nn.Linear(
            emb_size, self.channel * self.patch_size**2
        )
        ...
        self.features_proj = FeaturesCondition(input_dim=24, embed_dim=emb_size)
    ...
    def forward(self, input: torch.Tensor, t: torch.Tensor, text_input):
        ...
        x = x.view(
            x.size(0), self.patch_count, x.size(3)
        ...
        if text_input is not None:
            text_input_proj = self.features_proj(text_input)
            c = t + text_input_proj
        ...
        x = x.reshape(
            x.size(0), self.channel, self.H, self.W
